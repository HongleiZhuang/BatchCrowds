...
However, their could be correlations between annotations made by workers on data items within the same batch.  
...
The practices of crowdsourcing so far 
rarely pay attention to possible additional annotation error that might be introduced by grouping data items into batches.  
...
Typically they assume the labels are collected independently, which is not the case.  
