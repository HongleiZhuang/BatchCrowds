...
\agp{We don't need to say the previous sentence. Let's delete it. Replace with:
...
\vpara{Remarks.} 
1) Notice that a data item $x_i \in X$ may certainly appear in multiple batches in $A$.  
...
and associate each batch with a unique judgment made by different workers. 
...
\end{problem}
\agp{should the previous be $Y_{B_U}$?}
...
it is straightforward to extend our model when 
