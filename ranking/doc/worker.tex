...
\begin{itemize}
  ...
  If the worker is making judgments based on comparing data items within the same batch
...
but only $y_i \in \{0, 1\}$ is known,
as a heuristic, we can estimate $\eta_i$ by $\eta_i = (y_i + \epsilon)/(1 + 2 \epsilon)$
